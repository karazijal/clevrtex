...
class LitDTI(OOLLayeredBoxExp):
    ...
    def on_load_checkpoint(self, checkpoint) -> None:
        ...
        if "dti_train_metrics" in checkpoint:
            self.train_metrics = self.train_metrics.load_from_state_dict(
                checkpoint["dti_train_metrics"]
            )
        else:
    ...
    def lr_scheduler(self, opt):
        ...
        return (
            torch.optim.lr_scheduler.LambdaLR(opt, [lr_lambda_0, lr_lambda_1]),
            "step",
        )
