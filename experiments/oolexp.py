...
class OOLBoxExp(BaseExperiment, SpatialVisMixin):
    ...
    def training_step(self, batch, batch_idx):
        ...
        output = self.model(img, return_state=False)
        self.log(
            "loss", output["loss"], prog_bar=False, on_epoch=False, sync_dist=self.ddp
        )
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        mse = (
            F.mse_loss(output["canvas"], img, reduction="none")
            .sum((1, 2, 3))
            .mean()
            .detach()
        )
        ...
        if "wmask" in output["steps"]:
            w_mse = (
                F.mse_loss(
                    output["steps"]["mask"], output["steps"]["wmask"], reduction="none"
                )
                .sum((-1, -2, -3))
                .mean(-1)
                .mean()
        ...
        if len(other) == 1:
            ...
            cnts = (
                torch.round(masks.to(torch.float))
                .flatten(2)
                .any(-1)
                .to(torch.float)
                .sum(-1)
            ...
            self.log(
                prefix + "dice", mdice, prog_bar=True, on_epoch=True, sync_dist=self.ddp
            )

            aris = ari(ali_pmasks, ali_tmasks).mean().detach()
            self.log(
                prefix + "ari", aris, prog_bar=False, on_epoch=True, sync_dist=self.ddp
            )
        ...
        if "mc_expected_canvas" in output:
            mse = (
                F.mse_loss(output["mc_expected_canvas"], img, reduction="none")
                .sum((1, 2, 3))
                .mean()
                .detach()
            )
            self.log(
                prefix + "mc_mse", mse, prog_bar=True, on_epoch=True, sync_dist=self.ddp
            )
        ...
        if batch_idx == 0 and self.should_log_pictures():
            ...
            if "robj" in output["steps"] and "rmsk" in output["steps"]:
                self.log_slots(
                    output["steps"]["robj"][:32] * output["steps"]["rmsk"][:32],
                    prefix + "slots",
                )

            # If masks have been aligned; log semantic map
            if ali_pmasks is not None and ali_tmasks is not None:
                self.log_semantic_images(
                    img[:32], output["canvas"], ali_tmasks, ali_pmasks, prefix=prefix
                )
    ...
    def maybe_log_validation_outputs(self, batch, batch_idx, output, prefix=""):
        ...
        if len(other) == 1:
            ...
            if "steps" in output or "layers" in output:
                ...
                if not has_bg:
                    pred_masks_wbg = torch.cat(
                        [1 - pred_masks.sum(1, keepdim=True), pred_masks], 1
                    )
                else:
                ...
                self.onceprint(
                    "WARNING: Model returned neither 'steps' neither 'layers' in output"
                )
        ...
        if "counts" in output:
            ...
            pred_counts = (output["layers"]["mask"] > 0.5).flatten(2).any(
                -1
            ).detach().to(torch.int).sum(-1) - 1
        if pred_counts is not None:
            self.log(
                prefix + "acc",
                (pred_counts == cnts).to(float),
                prog_bar=True,
                on_epoch=True,
            )
            self.log(
                prefix + "cnt",
                pred_counts.to(float).mean(),
                prog_bar=True,
                on_epoch=True,
            )
        else:
            self.onceprint(
                "WARNING: No counts or layer masks in output; cannot track ACCURACY"
            )

        if "mc_expected_canvas" in output:
            mse = F.mse_loss(output["mc_expected_canvas"], img, reduction="none").sum(
                (1, 2, 3)
            )
        ...
        if batch_idx == 0 and self.should_log_pictures():
            ...
            if "steps" in output and "bbox" in output["steps"]:
                ...
                self.onceprint(
                    "WARNING: Neither steps with bbox or canvas_with_bbox found in output"
                )
            # Log input - output with background/slot breakdown underneath
            # self.log_recons(img[:32], output['canvas'],
            #                 output['steps']['patch'], output['steps']['mask'],
            ...
            if "mc_expected_canvas" in output:
                self.log_images(
                    img, output["mc_expected_canvas"], prefix=prefix + "mc_"
                )
            if "output_samples" in output:
                self.log_imgs_grid(img, *output["output_samples"], prefix=prefix)
