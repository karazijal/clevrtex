...
def is_docker():
    ...
    )  # TODO: should probably use something more sophisticated
...
def env_report():
    ...
    if torch.cuda.is_available():
        print(
            f"{print_prefix()} CUDA is available: see {torch.cuda.device_count()} GPU(s) using {torch.get_default_dtype()} by default"
        )
        if is_slurm():
            print(
                print_prefix(),
        ...
        print(
            f"{print_prefix()} CUDA is unavailable: using {torch.get_default_dtype()} by default"
        )
    if is_slurm():
