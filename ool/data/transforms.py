    ...
    def __call__(self, itm, *args):
        ...
        if len(rest) > 0 and rest[0].shape[-2:] == img.shape[-2:]:
            ...
            if torchvision.__version__.startswith("0.8"):
                ...
                masks = Ftv.resize(
                    masks, size=self.target_size, interpolation="nearest"
                )
            rest = (masks, *rest[1:])
        return (nimg, *rest)
    ...
    def __call__(self, itm, *args):
        ...
        masks = torch.cat(
            [
                masks[: self.n + 1],
                torch.zeros(
                    N - self.n - 1, C, H, W, dtype=masks.dtype, device=masks.device
                ),
            ],
            dim=0,
        )
        masks[0] = 1.0 - torch.any(masks[1:].to(bool), dim=0).to(torch.float)
