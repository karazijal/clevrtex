...
DATASETS = {
    ...
    "clevr_with_masks": {
        ...
        "ccrop_frac": 1.0,
        "variants": {None: {"shape": (3, 120, 160), "flags": {"div_mask"},}},
        "splits": {"test": (0, 15000), "val": (15000, 30000), "train": (30000, 1.0)},
    },
    "tetrominoes": {"fname": "tetrominoes.tar.gz", "variants": {None: {}}},
    "clevrtex": {
...
class ModDataset:
    ...
    def _resolve(dataset, split, dataset_variant=None):
        ...
        var = dataset_variant
        if (
            "splits" in meta["variants"][dataset_variant]
            and split in meta["variants"][dataset_variant]["splits"]
        ):
    ...
    def get_archive(dataset, split, dataset_variant=None):
        _, fname, *__ = ModDataset._resolve(
            dataset, split, dataset_variant=dataset_variant
        )
        return fname
    ...
    def __init__(self, path: Path, dataset, dataset_variant=None, split="train"):
        var, _, bias, limit = self._resolve(
            dataset, split, dataset_variant=dataset_variant
        )
        subfolder = dataset
        if var:
...
class WrappedCLEVRTEX(CLEVRTEX):
    ...
    def __getitem__(self, item):
        ...
        mask = (torch.arange(11, device=mask.device).view(-1, 1, 1, 1) == mask).to(
            torch.float
        )
