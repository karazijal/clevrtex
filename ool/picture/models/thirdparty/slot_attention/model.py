...
class SlotAttention(nn.Module):
    ...
    def forward(self, input):
        ...
        for _ in range(self.num_iterations):
            ...
            slots = self.gru(updates.reshape(-1, C), slots_prev.reshape(-1, C))
...
class SlotAttentionAutoEncoder(nn.Module):
    ...
    def __init__(self, input_shape, num_slots, num_iterations):
        ...
        self.layer_norm = nn.LayerNorm(
            [64, *resolution]
        )  # The original seems to normalise accross channels!
        self.mlp = nn.Sequential(
            nn.Conv2d(64, 64, 1), nn.ReLU(inplace=True), nn.Conv2d(64, 64, 1)
        )
    ...
    def forward(self, img):
        ...
        x = (
            slots[:, :, :, None, None]
            .expand(-1, -1, -1, *self.decoder_initial_size)
            .view(-1, slots.shape[-1], *self.decoder_initial_size)
        )
        # Now in [B*N, 64, h, w]
        ...
        if x.shape[-2:] != img.shape[-2:]:
            x = f.interpolate(
                x, size=img.shape[-2:], mode="bicubic", align_corners=False
            )
        # Now in [B*N, 4, H, W]
        ...
        r = {"canvas": recon_combined, "layers": {"mask": masks, "patch": recons,}}
        if self.training:
            r["loss"] = (
                f.mse_loss(recon_combined, img, reduction="none").sum((1, 2, 3)).mean()
            )
...
class SoftPositionEmbed(nn.Module):
    def __init__(self, hsize, resolution):
        ...
        h, w = resolution[-2:]
        hs = torch.linspace(0.0, 1.0, h)
        ws = torch.linspace(0.0, 1.0, w)
