...
class AverageMeter:
    ...
    def state_dict(self):
        return {"val": self.val, "sum": self.sum, "avg": self.avg, "count": self.count}
...
class Metrics:
    ...
    def __repr__(self):
        return ", ".join(
            ["{}={:.4f}".format(name, self.meters[name].avg) for name in self.names]
        )
...
def layered_composition_split(layers, masks, occ_grid):
    ...
    return (
        (occ_masks * masks * layers).sum(0),
        (occ_masks * masks).transpose(0, 1),
...
class DTISprites(nn.Module):
    ...
    def __init__(self, img_size, n_ch, n_sprites, n_objects=1, **kwargs):
        ...
        samples = torch.stack(
            [
                torch.full((n_ch, *(size or img_size)), 0.9, dtype=torch.float)
        ...
        self.mask_params = nn.Parameter(
            self.init_masks(
                n_sprites, kwargs.get("mask_init", "constant"), size, std
            ).clone()
        )
        self.cur_epoch = 0
        ...
        if self.has_layer_tsf:
            ...
            tsfs = [
                Transformer(n_ch, size, self.n_sprites, encoder=self.encoder, **kwargs)
                for k in range(L)
            ]
            ...
            tsfs = [
                Transformer(
                    n_ch, img_size, self.n_sprites, encoder=self.encoder, **kwargs
                )
                for k in range(L - 1)
            ]
        ...
        if self.learn_backgrounds:
            ...
            if bkg_init == "mean":
                ...
                bg_samples = torch.stack(
                    generate_data(None, M, init_type=bkg_init, value=0.5)
                )
    ...
    def forward(self, x):
        ...
        if class_prob is None:
            ...
            target, masks, layers = self.compose(
                tsf_layers, tsf_masks, occ_grid, tsf_bkgs, class_prob
            )  # BCHW
        ...
        return {
            ...
            "layers": {"mask": masks, "patch": layers},
        }
    ...
    def predict_occlusion_grid(self, x, features):
        ...
        if self.pred_occlusion:
            ...
            if self.occ_noise > 0 and self.training:
                N = int(L * (L - 1) / 2)
                occ_grid += (
                    2 * self.occ_noise * torch.rand(B, N, device=occ_grid.device)
                    - self.occ_noise
                )
    ...
    def greedy_algo_selection(self, x, layers, masks, bkgs, occ_grid):
        ...
        one, zero = (
            torch.ones(B, L, 1, device=device),
            torch.zeros(B, 1, L + 1, device=device),
        )
        occ_grid = torch.cat(
    ...
    def param_groups(self):
        return [
            ...
            {
                "params": list(self.transformer_parameters()),
                "lr": 1,
                "other": {"weight_decay": 1e-6},
            },
        ]
