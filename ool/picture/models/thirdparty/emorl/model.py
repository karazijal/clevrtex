...
from ool.picture.models.thirdparty.emorl.utils import (
    init_weights,
    mvn,
    std_mvn,
    GECO,
    gmm_negativeloglikelihood,
...
class ImageDecoder(nn.Module):
    ...
    def __init__(self, input_size, z_size, image_decoder, K, batch_size):
        ...
        if image_decoder == "big":
            ...
            self.positional_embedding = slot_attention_create_positional_embedding(
                small_grid_size, small_grid_size, K * batch_size
            )
    ...
    def forward(self, z):
        ...
        z_b = z.view((n, -1, 1, 1)).expand(
            -1, -1, self.z_grid_shape[0], self.z_grid_shape[1]
        )
...
class IndependentPrior(nn.Module):
    def __init__(self, z_size, K):
        ...
        self.z_size = z_size
        self.K = K
        self.z_linear = nn.Sequential(nn.Linear(self.z_size, 128), nn.ELU(True))
...
class RefinementNetwork(nn.Module):
    ...
    def __init__(self, z_size):
        ...
        self.encoding = nn.Sequential(
            nn.Linear(4 * z_size, 128), nn.ELU(True), nn.Linear(128, z_size)
        )
...
class GRUCell(jit.ScriptModule):
    ...
    def forward(self, input, hx):
        ...
        weight_in = self.pad(
            self.weight_in_1, self.weight_in_2, self.zeros_n
        )  # [2*hidden, 2*input_size]
        ...
        hy = (1.0 - updategate) * cellgate + updategate * hx  # [batch,2*hidden]
...
def slot_attention_create_positional_embedding(h, w, batch_size):
    ...
    return (
        torch.cat([dist_right, dist_left, dist_top, dist_bottom], 2)
        .unsqueeze(0)
...
class HVAENetworks(nn.Module):
    # @net.capture
    def __init__(
        self, K, z_size, input_size, stochastic_layers, use_DualGRU, batch_size
    ):
        ...
        self.encoder_pt_2 = nn.Sequential(
            nn.LayerNorm(64), nn.Linear(64, 64), nn.ReLU(True), nn.Linear(64, 64)
        )
        ...
        self.init_posterior = nn.Parameter(
    ...
    def forward(self, x, debug):
        ...
        lamda = self.init_posterior.repeat(
        ...
        for layer in range(self.num_stochastic_layers):
            ...
            all_samples[f"attn_{layer}"] = attn.view(
                self.batch_size, self.K, 1, self.H, self.W
            ).detach()
            ...
            slots_softplus = slots_softplus + self.mlp_softplus(
                self.norm_softplus_pre_ff(slots_softplus)
            )
...
class EfficientMORL(nn.Module):
    ...
    def __init__(
        ...
        self.hvae_networks = HVAENetworks(
            K=K,
            z_size=z_size,
            input_size=input_size,
        ...
        self.geco_C_ema = nn.Parameter(torch.tensor(0.0), requires_grad=False)
        self.geco_beta = nn.Parameter(torch.tensor(0.55), requires_grad=False)
        self.geco = GECO(
            self.training_kwargs["geco_reconstruction_target"],
            self.training_kwargs["geco_ema_alpha"],
        )
        self.kl_beta = self.training_kwargs["kl_beta_init"]

    def two_stage_inference(
        self, x, geco, global_step, kl_beta, get_posterior=False, debug=False
    ):
        total_loss = 0.0
        final_nll = 0.0
        final_kl = 0.0
        ...
        for refinement_iter in range(self.refinement_iters + 1):
            ...
            if refinement_iter > 0:
                ...
                delta_posterior_lamda, h = self.refinenet(
                    loss, posterior_lamda, h, not self.training
                )
                posterior_lamda = posterior_lamda + delta_posterior_lamda
                deltas += [
                    torch.mean(torch.norm(delta_posterior_lamda, dim=1)).detach()
                ]
            ...
            if self.image_likelihood == "GMM":
                ...
                nll = gaussian_negativeloglikelihood(
                    x_orig, torch.sum(x_loc * mask_logprobs.exp(), dim=1), log_var
                )
                ...
                if not self.bottom_up_prior:
                    ...
                    for layer in list(range(self.stochastic_layers))[::-1]:
                        ...
                        if layer == self.stochastic_layers - 1:
                            prior_z = std_mvn(
                                shape=[self.batch_size * self.K, self.z_size],
                                device=x.device,
                            )
                        else:
                            # z^l+1 ~ q(z^{l+1} | z^l, x)
                            z = posteriors[layer + 1].rsample()
                            loc_z, sp_z = self.hvae_networks.indep_prior(
                                z.view(-1, self.K, self.z_size)
                            )
            ...
            all_auxiliary[
                f"means_{(self.stochastic_layers - 1 +refinement_iter)}"
            ] = x_loc
            all_auxiliary[
                f"masks_{(self.stochastic_layers - 1 +refinement_iter)}"
            ] = mask_logprobs

            if kl_beta == 0.0 or self.geco_warm_start > global_step or geco is None:
                loss = torch.mean(nll + kl_beta * kl_div)
            else:
                loss = kl_beta * torch.mean(kl_div) - geco.constraint(
                    self.geco_C_ema, self.geco_beta, torch.mean(nll)
                )

            ## N.b. this is the opposite of IODINE; places more weight on the earlier losses than the later ones
            # from refinement
            total_loss += (
                (self.refinement_iters + 2 - refinement_iter + 1)
    ...
    def forward(self, x, global_step, debug=False):
        ...
        if self.training:
            for rf in range(
                len(self.training_kwargs["refinement_curriculum"]) - 1, -1, -1
            ):
                if global_step >= self.training_kwargs["refinement_curriculum"][rf][0]:
                    self.refinement_iters = self.training_kwargs[
                        "refinement_curriculum"
                    ][rf][1]
                    break

        auxiliary_outs, total_loss, nll, kl, deltas = self.two_stage_inference(
            x, self.geco, global_step, kl_beta=self.kl_beta, debug=debug
        )
    ...
    def update_geco(self, global_step, outs):
        if self.training and self.training_kwargs["use_geco"]:
            if global_step == self.geco_warm_start:
                ...
                self.geco_C_ema = self.geco.update_ema(
                    self.geco_C_ema, outs["rec_loss"]
                )
                self.geco_beta = self.geco.step_beta(
                    self.geco_C_ema,
                    self.geco_beta,
