...
class SPACEBackbone(nn.Sequential):
    ...
    def __init__(self, inc, extra_down=0):
        layers = [
            ...
            nn.GroupNorm(16, 128),
            nn.Conv2d(
                128, 256, 3, 1 if extra_down == 0 else 2, 1
            ),  # This has been changed from stride 1 to stride 2
            nn.CELU(),
            nn.GroupNorm(32, 256),
        ]
        for _ in range(1, extra_down):
            layers.extend(
                [nn.Conv2d(256, 256, 3, 2, 1), nn.CELU(), nn.GroupNorm(32, 256),]
            )
        layers.extend([nn.Conv2d(256, 128, 1), nn.CELU(), nn.GroupNorm(16, 128)])
